...
class _BanaiTabbarState extends State<BanaiTabbar> {
  ...
  void initState() {
    ...
    if (widget.unselectedLabelStyle != null && widget.unselectedLabelStyle!.fontSize==null) {
  ...
  Widget build(BuildContext context) {
    return TabBar(
      ...
      labelStyle:widget.labelStyle != null ? widget.labelStyle : TextStyle(),
  ...
  void updateTabviewAnimateValue(newValue){
    this.tabviewAnimateValue = newValue;
    // 判断向后移动，还是向前移动
    if(newValue > currentIndex) {
      nextInddex = currentIndex+1;
      diff = this.tabviewAnimateValue - (nextInddex - 1);
    } else {
      nextInddex = currentIndex-1;
      diff = nextInddex + 1 - this.tabviewAnimateValue;
    }
